...
import com.assessment.enums.FailureHandling;
...
public class WebUI {
    ...
    public static void setTextUsingJS(By by, String value) {
        smartWait();
    
        // Wait for the element to be visible
        WebElement element = DriverManager.getDriver().findElement(by);
    
        // Use JavaScriptExecutor to set the text
     ...
     * Click on the specified element.
     *
     * @param webElement an element of object type webelement
     */
    @Step("Click on the element {0}")
    public static void clickElement(WebElement element) {
        smartWait();
        element.click();
        LogUtils.info("Clicked on the element " + element.getText());

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Clicked on the element " + element.getText());
        }
        AllureManager.saveTextLog("Clicked on the element " + element.getText());
